...
fcl = {'tfc' : 6, 'sfc' : 1, 'hfc' : 1,'lfc' : 1} #特征选择参数
fs = {'p1':['tfc2','tfc3','tfc5','hfc1'],'p2':['tfc2','tfc5','hfc1'],'p3':['tfc2','tfc3','hfc1','lfc1'],
'p4':['tfc2','tfc3','hfc1','lfc1','sfc1'],'p4':['tfc2','tfc3','tfc5','hfc1','lfc1','sfc1']}#特征方案
# fs = {'p1' : ['tfc6','tfc3','hfc1']}
istrain = 1
dfn = 'df_tfc1'
...
def predicate(X_train,y_train,X_test):
    ...
    # fs = feature_selection.SelectKBest(feature_selection.f_regression, k=18)
    ...
    model = xgb.XGBRegressor(max_depth=5, learning_rate=0.1, n_estimators=160, silent=True, objective='reg:linear')
...
def hfc1(df,df_base): # 构造海拔特征:最大及最小值
    ...
    lable2 = pre + 'hmin'
    lable3 = pre + 'hmean'
    df_hight = df.groupby(['TERMINALNO'])['HEIGHT'].agg({lable1:max,lable2:min,lable3:np.mean}).reset_index()
...
def tfc2(df,df_base): #构造时间特征:平时及周末24小时内出行频度，即24小时内有几次高频出行
    ...
    df_whour[nhfreq] = t[t == True].count(axis=1)
...
def train(X,y):
    ...
    # X.fillna(0, inplace = True)
    Xs = X
    X_train, X_test, y_train, y_test = train_test_split(Xs, y, random_state=0, test_size=0.30)
    # params = {'n_estimators': 500, 'max_depth': 4,'learning_rate': 0.01, 'loss': 'ls'}
    # model = GradientBoostingRegressor(**params)
    ...
    print('gini:'+ str(gini(y_test,predicate)))
